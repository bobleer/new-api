...
const defaultGlobalSettingInputs = {
  ...
  'global.conversation_log_enabled': false,
          ...
          <Form.Section text={t('全局设置')}>
            <Row>
              ...
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.Switch
                  label={t('启用对话日志记录')}
                  field={'global.conversation_log_enabled'}
                  onChange={(value) =>
                    setInputs({
                      ...inputs,
                      'global.conversation_log_enabled': value,
                    })
                  }
                  extraText={t(
                    '开启后，将记录每次对话的完整消息和回复到JSON文件（需配置CONVERSATION_LOG_DIR环境变量）',
                  )}
                />
              </Col>
            </Row>
